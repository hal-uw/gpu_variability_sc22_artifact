...
1) LAMMPS input file for this test. 
  ...
  *There are three free varaibles in this file, three of which control the size of the simulation and the last will dictate how many MD time steps are taken.
  *The size of the system is controlled by the 'replicate' command given the values of $x, $y and $z.

  It is worth noting that these three free variables can be set at the command line when the simulation is invoked rather than editing the file by hand prior to each run.

  Example syntax:
    lmp_serial -in in.reaxc.hns -v x 2 -v y 2 -v z 2
