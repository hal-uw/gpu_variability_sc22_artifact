...
def showCurrentTemps(deviceList):
    ...
    @param deviceList: List of DRM devices (can be a single-item list)
    """
    printLogSpacer(str(time.time()) + ' Temperature ')
    for device in deviceList:
...
def relaunchAsSudo():
    ...
    """
    if os.geteuid() != 0:
        # note that the ROCM_PATH argument wasn't here to begin with, Prasoon added manually
        os.execvp('sudo', ['sudo', 'ROCM_PATH=/opt/rocm'] + sys.argv)
...
# Prasoon added this handler
def sigterm_handler(signalNumber, frame):
    print("SIGTERM being handled")
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    groupDisplayOpt.add_argument('-a', '--showallinfo', help='Show Temperature, Fan and Clock values',
                                 action='store_true')
    groupDisplayOpt.add_argument('--repeat', help='Repeat output every X seconds', type=float, metavar='SECONDS')
    ...
    try:
        while True:
            ...
            start_profile = time.clock_gettime_ns(time.CLOCK_MONOTONIC) 
            ...
            end_profile = time.clock_gettime_ns(time.CLOCK_MONOTONIC) 
            print('Profiling took %d ns' % (end_profile - start_profile))
            if args.repeat:
